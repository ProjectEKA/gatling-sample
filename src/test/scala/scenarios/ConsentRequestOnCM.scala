...
import scenarios.CreateConsentRequestOnCMForAPatient.UuidFeeder
...
object ConsentRequestOnCM {
  ...
  private val consnetDetail: String = "\"consent\":{\"" +
  ...
  val createConsentRequest: ChainBuilder = exec(http("Create consent request on CM")
      ...
      .body(StringBody("{\"requestId\": \"${uuid}\", \"timestamp\": \"" + LocalDateTime.now(ZoneOffset.UTC) + "\"," +
        consnetDetail))
      .check(status.is(202))
